...
def cluster_sequences(fastq_file, include_next_nearest=False):
    ...
    """
    # Maps sequences to cluster IDs - only contains actual sequences we've seen
    seq_to_cluster = {}
    # Cluster metadata
    clusters = []
    ...
    with open(fastq_file, 'r') as f:
        while True:
            ...
            if processed_sequences % 1000000 == 0:
                print(f"Processed {processed_sequences:,} sequences, {len(clusters)} clusters, {len(seq_to_cluster):,} reference sequences")
            ...
            if seq in seq_to_cluster:
                ...
                if found_cluster is not None:
                    ...
                    #if 'TTTCGGCTGTTAGGGGAGCCACCCC' in clusters[cluster_id]['sequences']:
                    #    print(clusters[cluster_id]['sequences'])

                    # IMPORTANT: Add this sequence as a reference for future matching
                    # This maintains the same clustering behavior as the original
                    ...
                    # Add this sequence as a reference for future matching
                    seq_to_cluster[seq] = cluster_id

    print(f"Processed {processed_sequences:,} valid sequences")
    print(f"Skipped {skipped_sequences:,} sequences with invalid characters")
    print(f"Final reference dictionary size: {len(seq_to_cluster):,} sequences")
...
def write_clustered_fasta(clusters, output_file):
    ...
    with open(output_file, 'w') as f:
        for cluster in sorted_clusters:
            ...
            f.write(f">{header} Nmembers:{len(cluster['sequences'])} N:{total_count}\n{seq}\n")
...
def main():
    ...
    parser.add_argument('-o', '--output', default='merged_lenfilt_cluster_lowmem.fasta',
                       help='Path to output FASTA file (default: merged_lenfilt_cluster_lowmem.fasta)')
